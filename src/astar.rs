...
where
    Node: Eq + Hash + Copy,
    Distance: Default + Add<Output = Distance> + Ord + Copy,
    I: Iterator<Item = Node>,
{
    // Dijkstra
    let mut boundary_nodes = HashSet::from([start]);
    let mut distances = HashMap::from([(start, Distance::default())]);
    ...
    while !boundary_nodes.is_empty() {
        ...
        for neighbor in neighbors(cur_node) {
            let proposed_distance = distances[&cur_node] + neighbor_edge_weight(cur_node, neighbor);
